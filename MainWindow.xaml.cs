...
// </copyright>
// ADAPTING BY GEORIGNA AGUILAR FOR PROTOTYPE OF PROJECT MAJOR DEGREE FOCUS ON ART
// VERSION 11. CONNECT WITH DB AND USER INTERFACE 
// APRIL 02-2016
//------------------------------------------------------------------------------
...
namespace Microsoft.Samples.Kinect.FaceBasics
{
    ...
    public partial class MainWindow : Window, INotifyPropertyChanged
    {
        ...
        //uuid(identificador unico para experimento)
        ...
        ///para validar combobox de contenido de imagenes
        //private string num_kinect;
        //private string imagen;

        //identificador de sensor kinect unico
        private string clave_kinect;

        //conexion a la base de datos
        FaceBasics.connetion conn = new FaceBasics.connetion();
        ...
        public MainWindow()
        {
            ...
            //para generar la clave unica por evento
            string clave_unica;
            ...
            //nombre de la computadora que captura experimento
            nombrePC = Environment.MachineName;
            nom_pc.Content = nombrePC;

            //id de kinect que captura el experimento
            clave_kinect = this.kinectSensor.UniqueKinectId;
            lbl_idKinect.Content = clave_kinect;
        }
        ...
        private void DrawFaceFrameResults(int faceIndex, FaceFrameResult faceResult, DrawingContext drawingContext)
        {
            ...
            //muestra los colores de los usuarios como identificador de captura de individuos
        ...
        //resta para sacar diagonal , es decir centro
            ...
            //interaccion con base de datos para insercion de datos capturados por el lente
            FaceFrameResult cara = faceResult;
            MySqlCommand command = new MySqlCommand();
            command.CommandType = System.Data.CommandType.Text;
            command.CommandText = "INSERT INTO exp2(clave_unica,clave_kinect,Happy,Engage,WearingGlasses,LeftEyeClosed,RightEyeClosed,MouthOpen,MouthMoved,LookingAway,nombrePC,Resource_Id,No_Usuarios,FaceBoxSourceLeft,FaceBoxSourceTop,FaceBoxSourceRight,FaceBoxSourceBottom,PointFX,PointFY,distanciaZ) VALUES (@clave_unica,@clave_kinect,@Happy,@Engaged,@WearingGlasses,@LeftEyeClosed,@RightEyeClosed,@MouthOpen,@MouthMoved,@LookingAway,@nombrePC,@Resource_Id,@No_Usuarios,@FaceBoxSourceLeft,@FaceBoxSourceTop,@FaceBoxSourceRight,@FaceBoxSourceBottom,@PointFX,@PointFY,@distanciaZ);";

            //campos de captura e insercion
            command.Parameters.Add("@clave_unica", MySqlDbType.VarChar, 200);
            command.Parameters["@clave_unica"].Value = strGuid;

            command.Parameters.Add("@clave_kinect", MySqlDbType.VarChar, 64);
            command.Parameters["@clave_kinect"].Value = this.kinectSensor.UniqueKinectId;
            ...
            command.Parameters["@Resource_Id"].Value = no_img.SelectionBoxItem;

            //extras para saber cerca o lejos estan de la obre
            command.Parameters.Add("@FaceBoxSourceLeft", MySqlDbType.UInt16);//base de datos
            command.Parameters["@FaceBoxSourceLeft"].Value = faceBoxSource.Left;// variable a mandar a bd refaceBoxSource.Left;

            command.Parameters.Add("@FaceBoxSourceTop", MySqlDbType.UInt16);
            command.Parameters["@FaceBoxSourceTop"].Value = faceBoxSource.Top;

            command.Parameters.Add("@FaceBoxSourceRight", MySqlDbType.UInt16);
            command.Parameters["@FaceBoxSourceRight"].Value = faceBox.Right;

            command.Parameters.Add("@FaceBoxSourceBottom", MySqlDbType.UInt16);
            command.Parameters["@FaceBoxSourceBottom"].Value = faceBox.Bottom;

            //puntos en X y Y
            command.Parameters.Add("@PointFX", MySqlDbType.UInt16);
            command.Parameters["@PointFX"].Value = faceBox.X;

            command.Parameters.Add("@PointFY", MySqlDbType.UInt16);
            command.Parameters["@PointFY"].Value = faceBox.Y;

            //resta entre puntos distancia
            command.Parameters.Add("@distanciaZ", MySqlDbType.UInt16);
        ...
        private void btn_inicioReinicio_Click(object sender, RoutedEventArgs e)
        {
        //genera un numero nuevo cada que se detiene la captura, es decir cada numero es una captura diferente
            //if (sesion == true)
            //{
            //    num_kinect = no_kinect.Text;
            //    imagen = no_img.Text;

            //    //clave_unica = System.Guid.NewGuid().ToString().Replace("-", "").ToUpper();
            //    sesion = false;
            //}
            //else
            //{
            ...
            if (this.bodyFrameReader.IsPaused)
            {
                ...
                this.btn_inicioReinicio.Content = "Pause";

            }
            else
            {
                this.bodyFrameReader.IsPaused = true;
                this.btn_inicioReinicio.Content = "Grabar";
